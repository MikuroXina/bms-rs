...
impl<'a> Token<'a> {
    pub(crate) fn parse(c: &mut Cursor<'a>) -> Result<Self> {
        loop {
            let command = c
                .next_token()
                .ok_or_else(|| c.generate_err_expected_token("command"))?;
            ...
            break Ok(match command.to_uppercase().as_str() {
                ...
                "#DIFFICULTY" => Self::Difficulty(
                    c.next_token()
                        .ok_or_else(|| c.generate_err_expected_token("difficulty"))?
                        .parse()
                        .map_err(|_| c.generate_err_expected_token("integer"))?,
                ),
                "#STAEGFILE" => Self::StageFile(
                    c.next_token()
                        .map(Path::new)
                        .ok_or_else(|| c.generate_err_expected_token("stage filename"))?,
                ),
                "#BANNER" => Self::Banner(
                    c.next_token()
                        .map(Path::new)
                        .ok_or_else(|| c.generate_err_expected_token("banner filename"))?,
                ),
                "#BACKBMP" => Self::BackBmp(
                    c.next_token()
                        .map(Path::new)
                        .ok_or_else(|| c.generate_err_expected_token("backbmp filename"))?,
                ),
                "#TOTAL" => Self::Total(
                    c.next_token()
                        .ok_or_else(|| c.generate_err_expected_token("gauge increase rate"))?,
                ),
                "#BPM" => Self::Bpm(
                    c.next_token()
                        .ok_or_else(|| c.generate_err_expected_token("bpm"))?,
                ),
                "#PLAYLEVEL" => Self::PlayLevel(
                    c.next_token()
                        .ok_or_else(|| c.generate_err_expected_token("play level"))?
                        .parse()
                        .map_err(|_| c.generate_err_expected_token("integer"))?,
                ),
                ...
                "#RANDOM" => {
                    let rand_max = c
                        .next_token()
                        .ok_or_else(|| c.generate_err_expected_token("random max"))?
                        .parse()
                        .map_err(|_| c.generate_err_expected_token("integer"))?;
                ...
                "#IF" => {
                    let rand_target = c
                        .next_token()
                        .ok_or_else(|| c.generate_err_expected_token("random target"))?
                        .parse()
                        .map_err(|_| c.generate_err_expected_token("integer"))?;
                    Self::If(rand_target)
                }
                "#ENDIF" => Self::EndIf,
                "#STAGEFILE" => {
                    Self::StageFile(c.next_token().map(Path::new).ok_or_else(|| {
                        c.generate_err_expected_token("splashscreen imege filename")
                    })?)
                }
                "#VOLWAV" => {
                    let volume = c
                        .next_token()
                        .ok_or_else(|| c.generate_err_expected_token("volume"))?
                        .parse()
                        .map_err(|_| c.generate_err_expected_token("integer"))?;
                    Self::VolWav(Volume {
                ...
                wav if wav.starts_with("#WAV") => {
                    ...
                    if str.is_empty() {
                        return Err(c.generate_err_expected_token("key audio filename"));
                    }
                ...
                stop if stop.starts_with("#STOP") => {
                    ...
                    let stop = c
                        .next_token()
                        .ok_or_else(|| c.generate_err_expected_token("stop beats"))?
                        .parse()
                        .map_err(|_| c.generate_err_expected_token("integer"))?;
                ...
                scroll if scroll.starts_with("#SCROLL") => {
                    ...
                    let scroll = c
                        .next_token()
                        .ok_or_else(|| c.generate_err_expected_token("scroll factor"))?;
                ...
                speed if speed.starts_with("#SPEED") => {
                    ...
                    let scroll = c
                        .next_token()
                        .ok_or_else(|| c.generate_err_expected_token("spacing factor"))?;
                    Self::Speed(ObjId::from(id, c)?, scroll)
                }
                ext_message if ext_message.starts_with("#EXT") => {
                    let message = c
                        .next_token()
                        .ok_or_else(|| c.generate_err_expected_token("message definition"))?;
                    ...
                    let track = message[1..4]
                        .parse()
                        .map_err(|_| c.generate_err_expected_token("[000-999]"))?;
