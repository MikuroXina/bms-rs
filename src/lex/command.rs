...
impl PlayerMode {
    pub(crate) fn from(c: &mut Cursor) -> Result<Self> {
        Ok(match c.next_token() {
            ...
            Some("2") => Self::Two,
            Some("3") => Self::Double,
            _ => return Err(c.generate_err_expected_token("one of 1, 2 or 3")),
        })
    }
}
...
impl JudgeLevel {
    pub(crate) fn from(c: &mut Cursor) -> Result<Self> {
        Ok(match c.next_token() {
            ...
            Some("2") => Self::Normal,
            Some("3") => Self::Easy,
            _ => return Err(c.generate_err_expected_token("one of 0, 1, 2 or 3")),
        })
    }
}
...
impl ObjId {
    ...
    pub(crate) fn from(id: &str, c: &mut Cursor) -> Result<Self> {
        id.try_into()
            .map_err(|_| c.generate_err_expected_token("[0-9A-Za-z][0-9A-Za-z]"))
    }
...
impl Key {
    ...
    pub(crate) fn from(key: &str, c: &mut Cursor) -> Result<Self> {
        ...
        Ok(match key {
            ...
            "8" => Key6,
            "9" => Key7,
            _ => return Err(c.generate_err_expected_token("[1-9]")),
        })
    }
}
