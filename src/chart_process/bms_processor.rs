...
use crate::chart_process::{
    BaseBpm, ChartEvent, ChartProcessor, ControlEvent, NoticeableChartEvent, VisibleChartEvent,
    WavId, YCoordinate,
...
impl BmsProcessor {
    ...
    fn generate_barlines_for_bms(
        bms: &Bms,
        events_map: &mut BTreeMap<YCoordinate, Vec<NoticeableChartEvent>>,
        ...
        for track in 0..=last_obj_time.track().0 {
            ...
            if track_y <= max_y {
                ...
                let evp = NoticeableChartEvent::new(
                    id_gen.next_id(),
                    y_coord.clone(),
                    event,
...
fn precompute_activate_times(bms: &Bms, all_events: &AllEventsIndex) -> AllEventsIndex {
    ...
    let new_map: BTreeMap<YCoordinate, Vec<NoticeableChartEvent>> = all_events
        .as_map()
        .iter()
        .map(|(y_coord, events)| {
            let y = y_coord.value();
