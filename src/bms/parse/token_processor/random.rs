...
use crate::{
    bms::{
        error::{ControlFlowError, ParseWarning},
...
impl<R: Rng, N: TokenProcessor> RandomTokenProcessor<R, N> {
    fn visit_random(&self, args: &str) -> Result<Option<ParseWarning>, ControlFlowError> {
        let push_new_one = || {
            ...
            if activated && !range.contains(&generated) {
                return Err(ControlFlowError::RandomGeneratedValueOutOfRange {
                    expected: range,
                    actual: generated,
                });
            }
    ...
    fn visit_set_random(&self, args: &str) -> Result<Option<ParseWarning>, ControlFlowError> {
        let push_new_one = || {
    ...
    fn visit_if(&self, args: &str) -> Result<Option<ParseWarning>, ControlFlowError> {
        ...
        match top {
            ...
            _ => Err(ControlFlowError::UnexpectedControlFlow(
                "#IF must be on a random scope".to_string(),
            )),
        }
    }

    fn visit_else_if(&self, args: &str) -> Result<Option<ParseWarning>, ControlFlowError> {
        let top = self
            .state_stack
            .borrow()
            .last()
    ...
    fn visit_else(&self) -> Result<(), ControlFlowError> {
        let top = self
            .state_stack
            .borrow()
            .last()
    ...
    fn visit_end_if(&self) -> Result<(), ControlFlowError> {
        let top = self
            .state_stack
            .borrow()
            .last()
    ...
    fn visit_end_random(&self) -> Result<(), ControlFlowError> {
        let top = self
            .state_stack
            .borrow()
            .last()
    ...
    fn visit_switch(&self, args: &str) -> Result<Option<ParseWarning>, ControlFlowError> {
        let push_new_one = || {
            let max: BigUint = match args.parse().map_err(|_| {
                ParseWarning::SyntaxError(format!("expected integer but got {args:?}"))
            }) {
            ...
            if activated {
                if !range.contains(&generated) {
                    return Err(ControlFlowError::SwitchGeneratedValueOutOfRange {
                        expected: range,
                        actual: generated,
                    });
                }
    ...
    fn visit_set_switch(&self, args: &str) -> Result<Option<ParseWarning>, ControlFlowError> {
        let push_new_one = || {
            let generated = match args.parse().map_err(|_| {
                ParseWarning::SyntaxError(format!("expected integer but got {args:?}"))
            }) {
    ...
    fn visit_case(&self, args: &str) -> Result<Option<ParseWarning>, ControlFlowError> {
        let cond = match args
            .parse()
    ...
    fn visit_skip(&self) -> Result<(), ControlFlowError> {
        let top = self
            .state_stack
            .borrow()
            .last()
    ...
    fn visit_default(&self) -> Result<(), ControlFlowError> {
        let top = self
            .state_stack
            .borrow()
            .last()
    ...
    fn visit_end_switch(&self) -> Result<(), ControlFlowError> {
        let top = self
            .state_stack
            .borrow()
            .last()
...
impl<R: Rng, N: TokenProcessor> RandomTokenProcessor<R, N> {
    fn on_header(&self, name: &str, args: &str) -> Result<Option<ParseWarning>, ControlFlowError> {
        if self.relaxed {
    ...
    fn on_comment(&self, line: &str) -> Result<Option<ParseWarning>, ControlFlowError> {
