    ...
    fn on_message(
        ...
        match channel {
            Channel::Scroll => {
                ...
                let scroll_warnings = obj_ids.into_iter().flat_map(|(time, obj)| {
                    objects
                        .scroll_defs
                        .get(&obj)
                        .cloned()
                        .map_or_else(
                            ...
                            |factor| {
                                objects
                                    .push_scrolling_factor_change(
                                        ScrollingFactorObj { time, factor },
                                        prompter,
                                    )
                                    .err()
                ...
                parse_warnings.into_iter().chain(scroll_warnings).collect()
            }
            _ => Vec::new(),
        }
    }
