...
impl WavProcessor {
    fn on_header(
        ...
        if name.eq_ignore_ascii_case("LNOBJ") {
            ...
            let _old_begin_kind = begin_note.channel_id.note_kind().ok_or_else(|| {
                ParseWarning::SyntaxError(format!(
                    "channel of specified note for LNOBJ cannot become LN {end_id:?}"
                ))
            })?;
            begin_note.channel_id = begin_note
                .channel_id
                .with_note_kind(NoteKind::Long)
                .ok_or_else(|| {
                    ParseWarning::SyntaxError(format!(
                        "channel of specified note for LNOBJ cannot become LN {end_id:?}"
                    ))
            ...
            let _old_end_kind = end_note.channel_id.note_kind().ok_or_else(|| {
                ParseWarning::SyntaxError(format!(
                    "channel of specified note for LNOBJ cannot become LN {end_id:?}"
                ))
            })?;
            end_note.channel_id = end_note
                .channel_id
                .with_note_kind(NoteKind::Long)
                .ok_or_else(|| {
                    ParseWarning::SyntaxError(format!(
