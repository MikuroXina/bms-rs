...
fn extract_random_block<'a>(
    ...
    tokens.push(Token::from(ControlFlowToken::Random(random_value)).into_wrapper(value));
    ...
    for IfBlock { branches, end_if } in if_blocks {
        for (branch_key, units) in branches {
            ...
            let if_token = Token::from(ControlFlowToken::If(branch_key)).into_wrapper(&units);
            ...
            tokens.push(Token::from(ControlFlowToken::EndIf).into_wrapper(&end_if));
        }
    }

    // Add the EndRandom token at recorded position
    tokens.push(Token::from(ControlFlowToken::EndRandom).into_wrapper(end_random));
...
fn extract_switch_block<'a>(
    ...
    tokens.push(Token::from(ControlFlowToken::Switch(switch_value)).into_wrapper(value));
    ...
    for CaseBranch { value, units } in cases {
        match value.content().clone() {
            CaseBranchValue::Case(case_value) => {
                // Add the Case token
                tokens.push(
                    Token::from(ControlFlowToken::Case(case_value.clone())).into_wrapper(&value),
                );
                ...
                let skip_token =
                    Token::from(ControlFlowToken::Skip).into_wrapper_range(skip_end..skip_end);
                tokens.push(skip_token);
            }
            CaseBranchValue::Def => {
                // Add the Def token
                tokens.push(Token::from(ControlFlowToken::Def).into_wrapper(&value));
    ...
    tokens.push(Token::from(ControlFlowToken::EndSwitch).into_wrapper(end_sw));
...
mod tests {
    ...
    fn test_extract_random_block() {
        use crate::bms::lex::token::{ControlFlowToken, Token};
        let if_tokens = vec![
        ...
        assert_eq!(
            extracted
                ...
                .collect::<Vec<_>>(),
            vec![
                Token::from(ControlFlowToken::Random(BigUint::from(1u32))),
    ...
    fn test_extract_switch_block() {
        ...
        assert_eq!(
            extracted
                ...
                .collect::<Vec<_>>(),
            vec![
                Token::from(ControlFlowToken::Switch(BigUint::from(1u32))),
    ...
    fn test_extract_multiple_random_branches() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                Token::header("TITLE", "Branch1_Token2"),
                Token::from(ControlFlowToken::EndIf),
                Token::from(ControlFlowToken::If(BigUint::from(2u32))),
                Token::header("TITLE", "Branch2_Token1"),
                Token::header("TITLE", "Branch2_Token2"),
                Token::from(ControlFlowToken::EndIf),
                Token::from(ControlFlowToken::EndRandom),
    ...
    fn test_extract_multiple_switch_cases() {
        ...
        assert_eq!(
            extracted
                ...
                .collect::<Vec<_>>(),
            vec![
                Token::from(ControlFlowToken::Switch(BigUint::from(3u32))),
                Token::from(ControlFlowToken::Case(BigUint::from(1u32))),
                Token::header("TITLE", "Case1_Token1"),
                Token::header("TITLE", "Case1_Token2"),
                Token::from(ControlFlowToken::Skip),
