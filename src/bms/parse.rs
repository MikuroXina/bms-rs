...
pub struct BmsParseTokenIter<'a>(
    std::iter::Peekable<std::slice::Iter<'a, SourcePosMixin<Token<'a>>>>,
);
...
impl<'a> BmsParseTokenIter<'a> {
    ...
    /// Create iter from SourcePosMixin<Token> list reference.
    pub fn from_tokens(value: &'a [SourcePosMixin<Token<'a>>]) -> Self {
...
impl<'a, T: AsRef<[SourcePosMixin<Token<'a>>]> + ?Sized> From<&'a T> for BmsParseTokenIter<'a> {
...
impl<'a> Deref for BmsParseTokenIter<'a> {
    type Target = std::iter::Peekable<std::slice::Iter<'a, SourcePosMixin<Token<'a>>>>;
