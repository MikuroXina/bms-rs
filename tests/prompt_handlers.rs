...
fn test_always_use_older() {
    ...
    let tokens: Vec<PositionWrapper<Token>> = vec![
    ...
    .into_iter()
    .map(|content| PositionWrapper::<Token> {
        content,
        row: 0,
        column: 0,
    })
    ...
    let BmsParseOutput {
        ...
        parse_warnings,
        ..
    } = parse_bms_step_model(&tokens, AlwaysUseOlder);
...
fn test_always_use_newer() {
    ...
    let BmsParseOutput {
        ...
        parse_warnings,
        ..
    } = parse_bms_step_model(&tokens, AlwaysUseNewer);
...
fn test_always_warn_and_use_older() {
    ...
    let BmsParseOutput {
        ...
        parse_warnings,
        ..
    } = parse_bms_step_model(&tokens, AlwaysWarnAndUseOlder);
    ...
    assert!(
        parse_warnings
            .iter()
            .all(|w| matches!(w.content, ParseWarning::HasDuplication))
    );
...
fn test_always_warn_and_use_newer() {
    ...
    let BmsParseOutput {
        ...
        parse_warnings,
        ..
    } = parse_bms_step_model(&tokens, AlwaysWarnAndUseNewer);

    // Should have no warnings since AlwaysWarnAndUseNewer handles conflicts silently
    assert!(
        parse_warnings
            .iter()
            .any(|w| matches!(w.content, ParseWarning::HasDuplication))
    );
