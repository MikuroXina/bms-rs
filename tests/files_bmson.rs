...
fn test_parse_bmson_with_negative_resolution() {
    ...
    let bmson = parse_bmson(json).expect("Failed to parse BMSON");
    assert_eq!(bmson.info.resolution, NonZeroU64::new(120).unwrap());
}

#[test]
fn test_parse_bmson_with_null_resolution() {
    let json = r#"{
        "version": "1.0.0",
        "info": {
            "title": "Test Song",
...
#[test]
fn test_parse_bmson_with_valid_resolution() {
    let json = r#"{
        "version": "1.0.0",
        "info": {
            "title": "Test Song",
    ...
    let bmson = parse_bmson(json).expect("Failed to parse BMSON");
    assert_eq!(bmson.info.resolution, NonZeroU64::new(480).unwrap());
}

#[test]
fn test_parse_bmson_with_large_negative_resolution() {
    let json = r#"{
        "version": "1.0.0",
        "info": {
            "title": "Test Song",
            ...
            "judge_rank": 100.0,
            "total": 100.0,
            "resolution": -9223372036854775808
        },
        "sound_channels": []
    }"#;
    ...
    assert_eq!(
        ...
        NonZeroU64::new(9223372036854775808).unwrap()
    );
}
