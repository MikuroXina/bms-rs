use bms_rs::{
    lex::{BmsLexOutput, LexWarning, parse, command::ObjId},
...
fn test_lal() {
    ...
    assert_eq!(bms.header.title.as_deref(), Some("Lilith ambivalence lovers"));
    assert_eq!(bms.header.artist.as_deref(), Some("ikaruga_nex (obj:Mikuro Xina)"));
    ...
    assert_eq!(bms.header.rank, Some(bms_rs::lex::command::JudgeLevel::Easy));
...
fn test_nc() {
    ...
    assert_eq!(bms.header.artist.as_deref(), Some("Silentroom obj: Mikuro Xina"));
    ...
    assert_eq!(bms.header.stage_file.as_ref().map(|p| p.to_string_lossy()), Some("stagefile.png".into()));
    assert_eq!(bms.header.banner.as_ref().map(|p| p.to_string_lossy()), Some("banner.png".into()));
...
fn test_j219() {
    ...
    assert_eq!(bms.header.artist.as_deref(), Some("cranky (obj: Mikuro Xina)"));
    ...
    assert_eq!(bms.header.stage_file.as_ref().map(|p| p.to_string_lossy()), Some("J219title.bmp".into()));
...
fn test_bemuse_ext() {
    ...
    assert_eq!(bms.header.scrolling_factor_changes.get(&ObjId::try_from("01").unwrap()), Some(&1.0));
    assert_eq!(bms.header.scrolling_factor_changes.get(&ObjId::try_from("02").unwrap()), Some(&0.5));
    assert_eq!(bms.header.spacing_factor_changes.get(&ObjId::try_from("01").unwrap()), Some(&1.0));
    assert_eq!(bms.header.spacing_factor_changes.get(&ObjId::try_from("02").unwrap()), Some(&0.5));
